...
parseBye = (string "BYE" *> AP.takeWhile (/= '\r')) Data.Functor.$> Bye
...
parseStatus = do
  ...
  return $ fmap (StatusR mailboxName) formattedStatuses
...
parseCapabilityWithValue = do
  name <- fmap decodeUtf8 (AP.takeWhile1 isAtomChar)
  char '='
  ...
  case T.toLower name of
    ...
    "appendlimit" -> return $ fmap CAppendLimit valAsNumber
...
parseExpunge = do
  ...
  return $ fmap Expunge (toInt msgId)
